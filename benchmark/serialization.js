...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new rtmcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new rtmcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = rtmcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new rtmcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function rtmcoreTest() {
      block1 = rtmcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('rtmcore', rtmcoreTest, {maxTime: maxTime});
